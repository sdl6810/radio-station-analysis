...
def recordData(filepath, musicData):
	...
	if os.path.exists(filepath):
		...
		if (_date.month != filepath[18:20] and _date.day != filepath[20:22]):
			...
			while (i <= (len(musicData)/4)-1):
				...
				newSheet['E'+str(3+i)].value = musicData[4*i]	#time played: 0, 3, 6, 9, 12, ..., 3i
				newSheet['C'+str(3+i)].value = musicData[(4*i)+1]	#song played: 1, 4, 7, 10, 13, ...3i+1
				newSheet['D'+str(3+i)].value = musicData[(4*i)+3]	#artist played: 2, 5, 8, 11, 14, ...,3i+2
				i=i+1
	...
	if not os.path.exists(filepath):
		...
		while (i <= (len(musicData)/4)-1):
			...
			log['E'+str(3+i)].value = musicData[4*i]	#time played: 0, 3, 6, 9, 12, ..., 3i
			log['C'+str(3+i)].value = musicData[(4*i)+1]	#song played: 1, 4, 7, 10, 13, ...3i+1
			log['D'+str(3+i)].value = musicData[(4*i)+3]	#artist played: 2, 5, 8, 11, 14, ...,3i+2
			i=i+1
